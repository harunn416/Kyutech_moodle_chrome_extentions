...
async function insertTextAtCursor(courseID) {
  ...
  if (!memoExists) {
    ...
    try {
      ...
      const currentCourseNameHeader = document.querySelector(
        "header#page-header div#page-navbar ol a",
      );
      if (
        currentCourseNameHeader &&
        currentCourseNameHeader.textContent.trim() !== ""
      ) {
        currentCourseName = currentCourseNameHeader.textContent.trim();
      } else {
        const currentCourseNameInput = document.querySelector(
          "header#page-header div.mr-auto h1.h2",
        );
  ...
  memoList.forEach((memo) => {
    if (memo.courseID !== OTHER_NOTES_KEY) {
      // 「その他」のメモは最初に追加
      const option = document.createElement("option");
...
function saveCurrentMemo() {
  ...
  saveMemoJson(selectedCourseID, memoData)
    .then(() => {
      ...
      // コース名セレクトボックスの表示名も更新
      const selectedOption = courseSelect.querySelector(
...
export async function createOtherIfNotExist() {
  ...
  const otherExists = memoList.some(
    (memo) => memo.courseID === OTHER_NOTES_KEY,
  );
  // 「その他」のメモがない場合は新しく作成
  if (!otherExists) {
