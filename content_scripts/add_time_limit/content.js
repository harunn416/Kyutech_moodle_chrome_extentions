...
function searchSubmittionElement() {
  let SubmitsDivElement = document.querySelector(
    "div.pb-2[data-region='event-list-wrapper']",
  );
...
function addTimeLimit(SubmitsDivElement) {
  ...
  divChildren.forEach((divElement) => {
    if (divElement.getAttribute("data-region") == "event-list-content-date") {
      // 日付要素
      // 日付文字列を取得しdate要素に変換
      ...
      Array.from(divElement.children).forEach((divSubmission) => {
        ...
        if (timeMatch) {
          ...
          // 現在時刻との差を出す。
          let timeDefference = findDifferenceBetweenTwoTimes(
            timeLimitDate,
            new Date(),
          );
          ...
          if (timeDefference.positiveNegative == -1) {
            ...
            if (timeDefference.days >= 1) {
              // 1日以上余裕あり
              timeLimitDiv.style.color = "#269900";
            } else if (timeDefference.hours >= 6) {
              // 24 ~ 6時間
          ...
          if (timeDefference.days !== 0 || timeDefference.hours !== 0) {
            ...
            if (timeDefference.days !== 0) {
              daysAndHoursDiv.innerHTML += timeDefference.days + "日 ";
            }
            if (timeDefference.hours !== 0) {
              daysAndHoursDiv.innerHTML += timeDefference.hours + "時間";
            }
          ...
          minutesAndSecondsDiv.innerHTML =
            timeDefference.minutes + "分 " + timeDefference.seconds + "秒";
          timeLimitDiv.appendChild(minutesAndSecondsDiv);

          // 残り時間追加
          const insertLimittimeDiv = divSubmission.querySelector(
            "div.d-flex.timeline-name",
          );
          if (insertLimittimeDiv.querySelector("div.timeLimitDivCustomise")) {
            // 既存の残り時間は消して更新
            insertLimittimeDiv
              .querySelector("div.timeLimitDivCustomise")
              .remove();
          }
          insertLimittimeDiv.insertBefore(
            timeLimitDiv,
            insertLimittimeDiv.querySelector("small").nextElementSibling,
          );
        } else {
