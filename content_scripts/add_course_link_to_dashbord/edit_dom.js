...
export function editDomToAddCourseLink() {
  const assignmentElementSelector =
    'div#page div#page-content div.pb-2[data-region="event-list-wrapper"] div.timeline-event-list-item';
  observeElementAppearance(
    assignmentElementSelector,
    async () => {
      document
        .querySelectorAll(assignmentElementSelector)
        .forEach((assignmentElement) => {
          // すでに追加済みなら何もしない
          if (assignmentElement.dataset.courseLinkAdded === "true") return;

          // 課題IDと課題リンクを取得
          const assignmentLinkElement = assignmentElement.querySelector(
            "div.timeline-name div.event-name-container a",
          );
          let assignmentURL = "";
          let assignmentID = "";
          ...
          linkDiv.addEventListener("click", (e) => {
            openCourseLink(assignmentID, assignmentURL, e.target);
          });

          // divを追加
          assignmentElement
            .querySelector("div.event-name-container")
            .appendChild(linkDiv);
