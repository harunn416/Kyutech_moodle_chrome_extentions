...
export async function getCourseLink(assignmentID, URL, onErrorCallback = null) {
  ...
  if (doc.body.id.includes("course-view"))
    return response.url + `#module-${assignmentID}`;

  // コースリンクの抽出
  const courseLinkElement = doc.querySelector(
    "div#page div#topofscroll header#page-header div#page-navbar ol a",
  );
