...
import {
  getCourseLinkFromLocalStorage,
  saveCourseLinkToLocalStorage,
} from "./operate_storage_local.js";
 ...
 */
export async function openCourseLink(
  assignmentId,
  assignmentLink,
  buttonElement,
) {
  ...
  if (courseLink) {
    ...
    const courseLink = await getCourseLink(assignmentId, assignmentLink, () => {
    ...
    // 課題IDがない場合は保存しない
    if (assignmentId !== "")
      saveCourseLinkToLocalStorage(assignmentId, courseLink);
