import {
  ...
  updateTimetableAtStorage,
  updateTimetable,
  deleteTimetableAtStorage,
} from "./content.js";
...
async function saveCourseJsonFromTimetable() {
  ...
  if (document.querySelector("#courseChangeNameInput").value !== "") {
    ...
    const courseID =
      document.querySelector("#courseEditNameDiv").dataset.courseId;
...
export function createPageEditPopup() {
  ...
  courseEditNameDayAndPeriod.innerHTML =
    "<span><span id='courseEditDay'></span>曜日</span><span> <span id='courseEditPeriod'></span>限目</span>";
...
function hideEditPopup() {
  document.querySelector("#courseEditNameDiv").style.display = "none"; // コース名を編集する部分のdivを閉じる(ポップアップは閉じない)
  document.querySelector("#courseChangeNameInput").value = ""; // 入力フィールドをクリア
  document.querySelector("#extension-edit-popup-overlay").style.display =
    "none";
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (let j = 0; j < 6; j++) {
      ...
      if (date_class_data["name"] != "") {
        ...
        date_class_element_span.setAttribute(
          "class",
          "course-edit-link courseLink",
        );
...
function apeardNameEditPopup(courseInformationJson) {
  ...
  document
    .querySelector("#courseEditNameDiv")
    .setAttribute("data-course-Link", courseInformationJson.courseLink);
  document
    .querySelector("#courseEditNameDiv")
    .setAttribute("data-course-id", courseInformationJson.courseID);
  document
    .querySelector("#courseEditNameDiv")
    .setAttribute("data-day", courseInformationJson.day);
  document
    .querySelector("#courseEditNameDiv")
    .setAttribute("data-period", courseInformationJson.period);

  //曜日と時限とコース名の情報を挿入
  document.querySelector("#courseEditDay").innerHTML = convertDayToJapanese(
    courseInformationJson.day,
  );
  document.querySelector("#courseEditPeriod").innerHTML =
    courseInformationJson.period;
  document.querySelector("#courseChangeNameInput").value =
    courseInformationJson.courseName;
  document.querySelector("#courseChangeLinkInput").value =
    courseInformationJson.courseLink;
}
