...
async function addCourseJsonFromTimetable() {
  ...
  const courseLink = document
    .querySelector("#divAddPop_TT")
    .getAttribute("data-course-Link");

  const courseSelectCheckboxes = document.querySelectorAll(
    "input.checkboxTimeschedule[type='checkbox']",
  );
...
export function createPageAddPopup() {
  ...
  courseNameInputTitle.innerHTML =
    "<label for='courseNameInput'>コースネーム<span style='font-size: 15px'> (短いほうがおすすめ)</span></label>";
  ...
  tableDescripsion.innerHTML =
    "<span>追加したい場所にチェックを入れてね！</span>";
...
function showAddPopup(courseInformationJson) {
  document.querySelector("#extension-add-popup-overlay").style.display =
    "block";
  document.querySelector("#divAddPop_TT").style.display = "block";

  document.querySelector("#courseNameInput").value =
    courseInformationJson.courseName;
  document.querySelector("#inputCourseId").innerHTML =
    courseInformationJson.courseID;
  document
    .querySelector("#divAddPop_TT")
...
export function setEventTimetableCustomiseButton() {
  const allTimetableCustomiseButton = document.querySelectorAll(
    "button.addCourseToTimetable",
  );
