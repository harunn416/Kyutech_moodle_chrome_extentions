    ...
    if (!timetableData) {
      ...
      try {
        ...
        await chrome.storage.sync.set({
          myUniversityTimetable: initialTimetableData,
        });
    ...
    if (!timetableData) {
      ...
      courseInformationIncludeTimeJson.times.forEach((timeJson) => {
        timetableData[timeJson.day][timeJson.period]["name"] =
          courseInformationIncludeTimeJson["courseInformation"]["name"];
        timetableData[timeJson.day][timeJson.period]["link"] =
          courseInformationIncludeTimeJson["courseInformation"]["link"];
        timetableData[timeJson.day][timeJson.period]["courseID"] =
          courseInformationIncludeTimeJson["courseInformation"]["courseID"];
      });
      // オブジェクトのキーと値のペアで保存
      // { '保存キー': 保存したい値 }
      await chrome.storage.sync.set({
...
async function displayCurrentCourse() {
  ...
  chrome.storage.sync.get("myUniversityTimetable").then(async (result) => {
    ...
    // 現在のコースを表示する要素を取得
    const currentCourseDisplay = document.getElementById(
      "currentCourseDisplay",
    );
    if (currentCourseDisplay === null)
      throw new Error("currentCourseDisplay element not found");
    ...
    if (typeof courseLink === "string" && courseLink !== "") {
      try {
        ...
        if (url.protocol === "http:" || url.protocol === "https:") {
          ...
          console.warn(
            "Unsafe protocol in courseLink detected, using fallback '#'.",
          );
        }
      } catch (e) {
    ...
    if (courseIndexObj.isBreakTime) {
      // 休憩時間中
      currentCourseDisplay.style.setProperty(
        "--progress",
        `${calculateBreakTimeProgress(courseIndexObj.index)}%`,
      );
    } else {
      // 授業時間中
      currentCourseDisplay.style.setProperty(
        "--progress",
        `${calculateCourseProgress(courseIndexObj.index)}%`,
      );
    }
  });
}
