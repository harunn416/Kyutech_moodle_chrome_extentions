...
import {
  getStoredLatestAccessedVersion,
  setStoredLatestAccessedVersion,
} from "./access_chrome_storage.js";
...
async function main() {
  ...
  if (currentVersionPlaceholder !== currentVersion) {
    console.warn(
      `⚠️ 警告: package.json のバージョン (${currentVersionPlaceholder}) と version_info.json の最新バージョン (${versionInfoArray[0].version}) が一致しません。`,
    );
    return;
  }
  ...
  if (storedVersion !== currentVersion) {
    console.log(
      `拡張機能が更新されました: ${storedVersion} → ${currentVersion}`,
    );
    // 更新情報ポップアップを表示
...
function createUpdatePopup(storedVersion, currentVersion) {
  ...
  const storedVersionDivided =
    storedIndex !== -1
      ? versionInfoArray[storedIndex].version.split(".")
      : [0, 0, 0];
