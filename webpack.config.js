...
try {
  ...
  const featureFolders = fs
    .readdirSync(contentScriptsPath, { withFileTypes: true })
        ...
        try {
          ...
          if (config.matches && Array.isArray(config.matches)) {
            ...
            if (
              ...
              // CSSファイルのパスをマップして追加
              manifestEntry.css = config.css.map(
                (cssFile) => `css/${folderName}/${cssFile}`,
              );
            ...
            console.warn(
              `警告: ${configFilePath} に 'matches' 配列が見つからないか無効です。`,
            );
          }
        } catch (parseError) {
          // config.json のJSONパースエラーが発生した場合の処理
          console.error(
            `エラー: ${configFilePath} のパースに失敗しました: ${parseError.message}`,
          );
        }
      } else {
        // config.json が見つからない場合の警告
        console.warn(
          `警告: ${folderName} ディレクトリ内に config.json が見つかりませんでした。content_scriptsエントリは生成されません。`,
        );
      }
    } else {
      // content.js または content.ts が見つからない場合の警告
      console.warn(
        `警告: ${folderName} ディレクトリ内に content.js または content.ts が見つかりませんでした。エントリポイントは生成されません。`,
      );
    }
  });
} catch (error) {
  ...
  console.error(
    `content_scripts ディレクトリの読み込み中にエラーが発生しました: ${error.message}`,
  );
            ...
            if (fs.existsSync(bundlePath)) {
              ...
              if (bundleContent.includes("__FEATURE_KEY_PLACEHOLDER__")) {
                ...
                bundleContent = bundleContent.replace(
                  /__FEATURE_KEY_PLACEHOLDER__/g,
                  featureKey,
                );
                console.log(
                  `✅ ${bundleFilename} に FEATURE_KEY を注入しました: ${featureKey}`,
                );
              }
              ...
              if (bundleContent.includes("__CURRENT_VERSION_PLACEHOLDER__")) {
                ...
                // 変数名に依存せずに置換できる
                bundleContent = bundleContent.replace(
                  /__CURRENT_VERSION_PLACEHOLDER__/g,
                  currentVersion,
                );
                fs.writeFileSync(bundlePath, bundleContent, "utf8");
                console.log(
                  `✅ ${bundleFilename} に CURRENT_VERSION を注入しました: ${currentVersion}`,
                );
              }
              fs.writeFileSync(bundlePath, bundleContent, "utf8");
            }
    ...
    // ビルド完了後に機能一覧JSONを生成するプラグイン
    {
      apply: (compiler) => {
        compiler.hooks.afterEmit.tap(
          "GenerateFeatureKeysJson",
          (compilation) => {
            // featureFolders変数から、機能名（キー）のリストを取得する
            const featureKeys = fs
              .readdirSync(contentScriptsPath, { withFileTypes: true })
            ...
            const features = featureKeys.map((key) => {
              ...
              if (fs.existsSync(configFilePath)) {
                try {
                  ...
                  console.error(
                    `エラー: ${configFilePath} のパースに失敗しました: ${e.message}`,
                  );
                }
              }
              return {
...
if (versionInfo[0].version !== packageJson.version) {
  console.warn(
    `⚠️ 警告: package.json のバージョン (${packageJson.version}) と version_info.json の最新バージョン (${versionInfo[0].version}) が一致しません。`,
  );
