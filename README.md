...
このファイルをダウンロードしてビルドしインストールすることもできます。その場合は常に最新版が使えますが未発見のバグもありますし手順もややこしいため、おすすめしません。  
**この場合は同じアカウント上で同期が正常に行われません。**
...
また、本開発のgit戦略に[`Gitflow`](https://qiita.com/KosukeSone/items/514dd24828b485c69a05)を採用しています。なるべく戦略に沿ったブランチを切っていただけると助かります。  
例えば、新機能追加の場合は`feature/[機能名]`といった感じにしてもらうと助かります。  
...
メモ機能でコースメインページ以外(提出ページなど)で別のコースIDを取得して別のメモを作成する現象を修正。
